...
| `[card] max_tool_output_chars` | `0` | `0` = 不输出 tool call 结果内容（默认）；`>0` 时结果收进工具折叠面板，超过则折叠，完整内容保留 |
| `[card] fold_long_output` | `true` | true：tool call 折叠成 collapsible_panel（默认收起）；false：内联全文（结果仍受 10240 硬上限约束） |
