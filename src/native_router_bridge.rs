...
//! `SessionManager`），而不是 acp 桥。飞书侧是唯一调用方：`RouterHandle`
//! 在 `on_text`/`PassThrough`/`Btw` 看到 `agent-*` key 时经
//! [`sebas_router::native_bridge::NativeSessionBridge`] 转发到这里。
//!
...
impl RouterNativeBridge {
    pub fn new(manager: Arc<SessionManager>, router: RouterHandle) -> Arc<Self> {
