#!/usr/bin/env bash
# sebas gateway e2e 验证脚本（Task 11 / sebas-lva.11）。
#
# 流程：cargo build → 生成临时 config → 起 sebas gateway → curl /healthz →
...
#   --keep-tmp  调试：保留临时目录与 gateway.log（默认清理）。
#
# 设计要点：
...
TMPDIR="$(mktemp -d -t sebas-gateway-e2e.XXXXXX)"
CONFIG="$TMPDIR/gateway.toml"
USAGE_FILE="$TMPDIR/gateway-usage.jsonl"
...
# ---- 清理：杀 gateway 进程 + 删临时目录。trap 覆盖所有退出路径。 ----
GATEWAY_PID=""
cleanup() {
...
# --keep-tmp：调试用，保留临时目录与 gateway.log。
KEEP_TMP=0
for arg in "$@"; do
  case "$arg" in
    --keep-tmp) KEEP_TMP=1 ;;
...
# ---- 3. 起 gateway ----
echo "[3/7] 启动 sebas gateway"
"$BIN" gateway --config "$CONFIG" >"$LOG_FILE" 2>&1 &
GATEWAY_PID=$!
ready=0
for _ in $(seq 1 50); do
  if curl -sS --max-time 1 "$BASE/healthz" >/dev/null 2>&1; then ready=1; break; fi
  kill -0 "$GATEWAY_PID" 2>/dev/null || { echo "error: gateway 进程已退出"; cat "$LOG_FILE"; exit 1; }
  sleep 0.2
done
if [[ $ready -ne 1 ]]; then
  echo "error: gateway 在 10s 内未就绪"; cat "$LOG_FILE"; exit 1
fi
echo "  gateway 就绪 (PID $GATEWAY_PID, listen $BASE)"
