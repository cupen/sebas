...
#                      alive via test-build), separate `gateway --debug` HTTP
#                      child on [gateway] listen 127.0.0.1:8787, control-plane
#                      Unix socket, per-run secret.
#   test sequence    : dry-run dev -> real dev -> dry-run rollback -> rollback
#   the gateway --debug child must stay up across every core restart.
set -u

REPO=/home/cupen/workbench/repos-tool/sebas
...
mkdir -p "$WORK"
SEBAS_IPC=1 \
"$EXE" watchdog --debug --config "$CFG" >"$LOG" 2>&1 &
WATCH_PID=$!
...
# alive; give it a moment. Confirm the debug gateway is sharing :8787.
echo "=== confirm debug gateway :8787 (independent child) ==="
sleep 2
curl -s -o /dev/null -w 'gateway GET / -> %{http_code}\n' http://127.0.0.1:8787/ 2>&1 || echo "(gateway not yet up)"
...
request update --dev
echo "gateway after dev-update (must still be up):"
...
echo "### final core pid:" $(pgrep -f 'run --config' 2>/dev/null | tr '\n' ' ')
