...
//!   into the `sebas_router::models::ModelDef` shape, and regenerate the
//!   `MODELS` const body in `sebas-router/src/models.rs`.
...
fn print_help() {
    ...
    println!("                    sebas-router/src/models.rs::MODELS in place.");
...
fn run_update_models(extra_args: &[String]) -> ExitCode {
    ...
    if extra_args.iter().any(|a| a == "--help" || a == "-h") {
        println!(
            "xtask update-models — fetch {} and regenerate \
             sebas-router/src/models.rs::MODELS",
            MODELS_DEV_URL
        );
    ...
    println!(
        "Updated {} entries, wrote sebas-router/src/models.rs (timestamp: {}); file marker: {}",
        report.entries_written, utc_now, report.timestamp_line
    );
    ExitCode::SUCCESS
}

/// Locate `sebas-router/src/models.rs` relative to the xtask crate's
/// `CARGO_MANIFEST_DIR`. xtask lives at `<repo>/xtask/`, the target
/// lives at `<repo>/sebas-router/src/models.rs`.
fn locate_models_rs() -> PathBuf {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
    PathBuf::from(manifest_dir)
        .parent()
        .map(|p| p.join("sebas-router/src/models.rs"))
        .unwrap_or_else(|| PathBuf::from("sebas-router/src/models.rs"))
}
