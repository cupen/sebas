...
//!
//! 之前 `RouterConfig::resolve_api_keys` 直接内联三种来源（env 变量 / 明文 /
...
/// 等实现独立测试 hint 派生逻辑，不必绕过 `RouterConfig::parse`。
...
mod tests {
    ...
    fn hint_from_provider_empty_strings_treated_as_missing() {
        // 和 `RouterConfig::resolve_providers` 一致：空字符串 ≡ 缺省。
