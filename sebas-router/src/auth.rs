//! 下游 token 鉴权中间件（见 openspec/specs/router-auth-rate-limit/spec.md）。
//!
//! `require_key` 以 `from_fn_with_state` 挂在 fallback 层上方，覆盖所有路由
//! （含 `/healthz`，由中间件内豁免）。`Authorization: Bearer <key>` 优先、
//! `x-api-key: <key>` 次之；缺失/未匹配 → 401，协议面由 `resolve_target` 嗅探，
//! 无法识别时默认 OpenAi。
//!
//! 安全铁律：401 message 恒为 `"invalid or missing API key"`，绝不回显呈现的
//! key，也不落日志。

use axum::extract::{Request, State};
use axum::http::{HeaderMap, StatusCode};
use axum::middleware::Next;
use axum::response::Response;

use crate::error::error_response;
use crate::proto::{WireProtocol, resolve_target};
use crate::server::AppState;

/// 从 `Authorization: Bearer <key>` 或 `x-api-key: <key>` 提取下游 key。
/// Authorization 优先；Bearer 令牌为空时回退 `x-api-key`。两者均无/空 → `None`。
/// scheme 大小写不敏感（`Bearer`/`bearer` 均可），首尾空白自动 trim。
/// `pub(crate)`：限流中间件（rate_limit.rs）复用同一把 key 做 per-token 维度。
pub(crate) fn extract_key(headers: &HeaderMap) -> Option<String> {
    if let Some(auth) = headers.get("authorization")
        && let Ok(s) = auth.to_str()
    {
        // scheme 大小写不敏感 + trim（`bearer <k>`、`Bearer  <k>  ` 均可）。
        let s = s.trim();
        if s.len() >= 6 && s[..6].eq_ignore_ascii_case("bearer") {
            let after = &s[6..];
            // 要求 scheme 后有空白分隔（防 `bearertoken` 误命中），token 非空。
            if after.starts_with(' ') || after.starts_with('\t') {
                let rest = after.trim().trim_start_matches(':').trim();
                if !rest.is_empty() {
                    return Some(rest.to_string());
                }
            }
        }
    }
    if let Some(k) = headers.get("x-api-key")
        && let Ok(s) = k.to_str()
    {
        let s = s.trim();
        if !s.is_empty() {
            return Some(s.to_string());
        }
    }
    None
}

/// `listen` 是否为 loopback 绑定（`127.0.0.1`/`localhost`/`::1`）。
/// 用于无鉴权时的公网裸奔门：与 webui 的非 loopback 启动门同理。
fn listen_is_loopback(listen: &str) -> bool {
    let host = listen
        .rsplit_once(':')
        .map(|(h, _)| h)
        .unwrap_or(listen)
        .trim()
        .trim_matches(|c| c == '[' || c == ']');
    host.eq_ignore_ascii_case("127.0.0.1")
        || host.eq_ignore_ascii_case("localhost")
        || host == "::1"
}

/// 渲染 401：协议面由 `resolve_target` 嗅探；非 `/v1` 路径 → 默认 OpenAi。
/// message 恒为通用串，绝不回显呈现的 key。
fn unauthorized(headers: &HeaderMap, path: &str) -> Response {
    crate::metrics::Metrics::global().observe_auth_rejected();
    let proto = resolve_target(headers, path)
        .map(|t| t.protocol)
        .unwrap_or(WireProtocol::OpenAiChat);
    let err_type = match proto {
        WireProtocol::Anthropic => "authentication_error",
        WireProtocol::OpenAiChat | WireProtocol::OpenAiResponses => "invalid_request_error",
    };
    error_response(
        proto,
        StatusCode::UNAUTHORIZED,
        err_type,
        "invalid or missing API key",
    )
}

/// 鉴权中间件。`/healthz` 豁免；`auth_token` 未配置时：
/// loopback 监听维持原行为（本地开发零摩擦，启动时已 warn），
/// 非 loopback 监听直接 401（与 webui 的非 loopback 启动门同理，
/// 防止公网误配成无鉴权）；配置后要求呈现的 token 命中集合才放行。
pub async fn require_key(State(state): State<AppState>, req: Request, next: Next) -> Response {
    // /healthz 豁免——layer 挂在 fallback 上方，故 /healthz 也过此中间件，
    // 由这里短路放行，不校验 key。
    if req.uri().path() == "/healthz" {
        return next.run(req).await;
    }
    // debug 模式（--debug，内置 test provider）：跳过下游鉴权。
    // auth_tokens 在可热替换内核里，取一次快照。
    let core = state.core();
    if core.cfg.debug {
        return next.run(req).await;
    }
    if core.auth_tokens.is_empty() {
        // 未配置 auth_token：loopback 放行（build_state 启动时已 warn），
        // 非 loopback 拒绝——公网裸奔只能是误配。
        if listen_is_loopback(&core.cfg.listen) {
            return next.run(req).await;
        }
        tracing::error!(
            "[router] non-loopback listen ({}) without auth_token: rejecting request",
            core.cfg.listen
        );
        return unauthorized(req.headers(), req.uri().path());
    }
    match extract_key(req.headers()) {
        Some(key) if core.auth_tokens.contains(&key) => next.run(req).await,
        _ => unauthorized(req.headers(), req.uri().path()),
    }
}
