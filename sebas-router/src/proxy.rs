//! 透传引擎（Task 7，见 openspec/specs/router-core/spec.md）。
//!
//! 纯透传路由核心：同协议转发，不改写协议体。`handle` 是 axum fallback
...
/// hop-by-hop + 下游 auth header：请求侧剥离集合（见 openspec/specs/router-core/spec.md）。
...
/// 路由自身 5xx（上游不可达 / 配置缺失）。通用 message，不含 key/内部细节。
...
pub async fn handle(State(state): State<AppState>, req: Request) -> Response {
                ...
                Err(e) => {
                    ...
                    let (status, err_type, message) = if is_length_limit {
                        (
                            ...
                            "request_too_large",
                            "request body exceeds router max_body_bytes",
                        )
                    } else {
                        (
                            StatusCode::BAD_REQUEST,
    ...
    let provider_cfg: &ProviderConfig = match core.cfg.providers.get(&decision.provider) {
        ...
        None => {
            return upstream_error_response(
                proto,
                "resolved provider not found in config (router state inconsistent)",
            );
        }
    };
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            // 路由侧失败：error 字段承载原因，status=502。token 字段全 None
            // （请求未到达上游，无 usage）。
            settle_inner(
                &state.sink,
                proto,
