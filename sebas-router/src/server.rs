//! axum server skeleton for the router (Task 2).
//!
...
pub struct RouterCore {
    ...
    /// 合法下游 token 集合（`[router] auth_token`，单串或数组）。
...
impl RouterCore {
    ...
    pub fn build(cfg: RouterConfig) -> Result<RouterCore> {
...
/// 路由相关字段经 `core`（`Arc<RwLock<RouterCore>>`）可热替换。
/// 每请求一次 `read()` 快照（clone Arc 引用，纳秒级临界区，用 std RwLock）。
#[derive(Clone)]
pub struct AppState {
    pub cfg: Arc<RouterConfig>,
    pub core: Arc<RwLock<RouterCore>>,
...
impl AppState {
    ...
    pub fn swap_core(&self, candidate: RouterConfig) -> Result<()> {
        let new_core = RouterCore::build(candidate)?;
        let mut guard = self
            .core
            .write()
            .unwrap_or_else(|e| e.into_inner());
...
/// at startup.
pub fn build_state(cfg: RouterConfig) -> Result<AppState> {
    let auth_tokens: HashSet<String> = cfg.auth_token.iter().cloned().collect();
    ...
    let client = reqwest::Client::builder()
        ...
        // contract calls out (openspec/specs/router-core/spec.md, 放宽到分钟级)
...
pub async fn run(cfg: RouterConfig) -> Result<()> {
    ...
    tracing::info!(%addr, "sebas router listening");
    axum::serve(
        listener,
...
async fn shutdown_signal() {
    ...
    tracing::info!("router shutdown signal received, draining connections");
}
...
mod tests {
    ...
    fn test_cfg(provider_names: &[&str]) -> RouterConfig {
