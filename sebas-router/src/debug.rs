//! Debug 模式注入路径。
//!
//! `enable_debug_test_provider` 仅在 `sebas router --debug` 与
//! `sebas run --debug` 路径调用，不属于生产配置语义。把它从 `config.rs`
//! 挪出，避免后续维护者疑惑为什么 prod config 解析里混着 debug 注入逻辑。
//!
//! 注入内容：provider 名 `test`、base_url `router://self` 哨兵值（proxy
...
"#;
        ...
        assert_eq!(test.base_url_anthropic.as_deref(), Some("router://self"));
