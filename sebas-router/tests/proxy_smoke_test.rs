//! 透传引擎 smoke test（Task 7，见 openspec/specs/router-core/spec.md）。
//!
//! 2 例（brief 契约）：
...
//!
//! 不复用 `support::start_router`：mini 上游需要把 provider.base_url 指向
...
async fn json_post_passthrough_injects_upstream_key_and_preserves_business_headers() {
    ...
    let gw = support::start_router(&router_cfg(upstream_addr)).await;
    let client = client();
...
async fn unreachable_upstream_returns_502_upstream_error() {
    ...
    // router 的 connect_timeout 默认 10s；这里把 reqwest 客户端 timeout 设 5s
