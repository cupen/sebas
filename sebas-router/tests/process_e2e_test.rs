//! 进程级 e2e：spawn 真实 `sebas router` 二进制，provider 指向 in-process
...
/// 定位 workspace 根 `target/debug`（router crate 的 manifest 在其子目录）。
...
"#,
    ...
    std::fs::write(&path, body).expect("write router config");
    path
}
...
fn write_top_level_provider_config(
    ...
    let usage = usage_path.to_string_lossy().replace('\\', "/");
    let path = dir.join("router-top-level.toml");
    let body = format!(
        r#"
[router]
listen = "127.0.0.1:{port}"
...
"#,
    ...
    std::fs::write(&path, body).expect("write top-level-provider router config");
    path
}

/// 运行中的 router 子进程。drop 时 kill + wait，不泄漏进程。
...
async fn real_binary_forwards_anthropic_openai_auth_and_usage() {
    ...
    let mut gw = RouterProcess {
        child: Command::new(&bin)
            .arg("router")
            .arg("--config")
            .arg(&config_path)
            .env("SEBAS_ROUTER_TEST_UPSTREAM_KEY", "test-anthropic-key")
            .env("SEBAS_ROUTER_TEST_UPSTREAM_KEY_OAI", "test-openai-key")
            .stdout(Stdio::null())
            .stderr(Stdio::piped())
            .spawn()
            .expect("spawn sebas router"),
    };
    ...
    let ready = tokio::time::timeout(Duration::from_secs(10), async {
        ...
        loop {
            ...
            if let Some(status) = gw.child.try_wait().expect("try_wait") {
                panic!(
                    "sebas router exited early with {status}: {}",
    ...
    if ready.is_err() {
        panic!(
            "sebas router not ready within 10s; stderr: {}",
            gw.kill_and_capture_stderr()
        );
    }
    ...
    if !stderr.is_empty() {
        eprintln!("sebas router stderr:\n{stderr}");
    }
}

#[tokio::test]
async fn standalone_router_reads_top_level_provider_table() {
    let bin = sebas_bin();
    if !bin.exists() {
        eprintln!(
            "skipping: {} missing (run `cargo build --bin sebas` first)",
    ...
    let mut gw = RouterProcess {
        child: Command::new(&bin)
            ...
            .spawn()
            .expect("spawn standalone sebas router"),
    };
    let base = format!("http://127.0.0.1:{port}");
    ...
    let resp = client
        ...
        .send()
        .await
        .expect("POST /v1/messages via standalone router");
    assert_eq!(resp.status(), reqwest::StatusCode::OK);
    assert_eq!(
        resp.headers().get("x-mock-trace").unwrap(),
        "anthropic-messages-json"
