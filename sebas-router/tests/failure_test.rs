...
/// router config：单 provider `up` 指向 ad hoc 上游，明文 api_key（仅测试用）。
/// `extra_router_fields` 插到 `[router]` 段（如 `read_timeout_secs`）。
fn cfg(upstream: &str, extra_router_fields: &str) -> String {
    format!(
        r#"
[router]
listen = "127.0.0.1:0"
...
async fn upstream_read_timeout_returns_502_without_key_leak() {
    ...
    let gw = start_router(&cfg(&url, "read_timeout_secs = 1\n")).await;
    let resp = post_messages(&client(), &format!("http://{}", gw.addr))
        .send()
        .await
        .expect("POST /v1/messages");
