...
`invoke testsuite-webui` SHALL 是套件唯一入口：构建（含 dist 自动重建）→ 装配沙箱 → 运行全部用例 → 清理；支持按名过滤单个旅程；后续补充用例 SHALL 经既有过滤机制运行、无需新增入口。断言 SHALL 基于状态轮询而非固定 sleep（web-first + 轮询，禁固定 sleep）；同一提交重复运行 SHALL 稳定收敛（多次全绿为收尾门槛），清理与保留现场行为一致。
