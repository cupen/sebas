# gateway-auth-rate-limit Specification

## Purpose
Defines the gateway's control plane: downstream API key authentication, the
...
who may talk to the gateway and what gets recorded, as opposed to the request
forwarding itself.
...
The gateway SHALL authenticate requests with a static key table built from
the `[gateway] auth_token` config (accepting either a single string or an
...
### Requirement: Open gateway when unconfigured

When no `auth_token` is configured (or the gateway runs in `--debug` mode),
the gateway SHALL skip authentication entirely and serve requests without a
key; such requests are attributed to the shared `anonymous` identity.

#### Scenario: no auth_token configured

- **WHEN** the gateway config has no `auth_token` and a keyless request
  arrives
- **THEN** the request proceeds to routing and rate limiting without a 401

### Requirement: Per-key token-bucket rate limiting

The gateway SHALL rate-limit per extracted key (anonymous bucket for keyless
requests on an open gateway) using a token bucket: capacity tokens, refilled
at `refill_per_sec`, starting full, with lazy refill capped at capacity.
Configuration under `[gateway.rate_limit]` accepts either `capacity` +
...
- **WHEN** `[gateway.rate_limit] rpm = 60` is configured
...
writer that appends JSONL to `~/.sebas/gateway-usage.jsonl` (parent directory
...
error field is populated only for gateway-side failures (e.g. upstream
connect failure → status 502); upstream 4xx/5xx responses record their
status with no gateway error.
...
#### Scenario: upstream error recorded without gateway error
...
For `text/event-stream` responses the gateway SHALL extract usage
...
For non-SSE responses the gateway SHALL parse usage from the fully buffered
...
line via `tracing` at target `gateway::access`, on response-body completion
