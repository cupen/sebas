...
(without setting a default model).
...
- **WHEN** the user clicks `Gateway`
- **THEN** the state file records mode `gateway` and the card refreshes with
  `Gateway` rendered as the active mode
...
state overlay, falling back to the gateway config's provider table when the
...
- **WHEN** Direct mode names provider `ghost` that exists neither in the
  overlay nor the gateway config
...
Gateway mode the `--model` flag SHALL never be added.
...
- **WHEN** mode is `gateway` and both default selection model and provider
  default model exist
- **THEN** no `--model` arg is passed

### Requirement: Gateway mode env translation

In `Gateway` mode the spawn SHALL translate the gateway's `listen` address
and first `auth_token` into `ANTHROPIC_BASE_URL=http://{listen}` +
`ANTHROPIC_AUTH_TOKEN={token}` — the gateway always presents the Anthropic
...
- **WHEN** mode is `gateway` with `listen = "127.0.0.1:8787"` and
  `auth_token = ["sk-x"]`
...
- **WHEN** mode is `gateway` and the gateway config has an empty `listen`
