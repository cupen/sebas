...
The watchdog SHALL spawn the core as `current_exe() core --config <path>` — the same binary, core subcommand, and the watchdog's own config — with piped stdio, `kill_on_drop`, and env `SEBAS_IPC=1` plus a per-instance `SEBAS_CONTROL_SECRET`. The core signals readiness over the pipe; a child exit is classified and the watchdog loops to respawn after a fixed 1000 ms delay. Spawn failures (missing binary, no stdio) after N consecutive failures (default 3, configurable via `[watchdog] max_spawn_failures`) SHALL enter the `failed-startup` terminal state — the service status SHALL be set to `failed-startup`, the watchdog process SHALL exit with EX_TEMPFAIL (75), and `sebas ctl status` SHALL report a failure summary (most recent stderr line plus the failure count). Within N failures the watchdog SHALL still retry with the 1 s backoff, but every failure SHALL write a structured error log (including a stderr summary) and SHALL surface to the operator through the existing reporting paths (systemd unit status, `sebas ctl status`, Feishu boot notifications when enabled). A core exit before ready due to an early-fatal line SHALL count toward the same `failed-startup` terminal-state counter; a startup failure of the new binary after a ready-after-rollback SHALL NOT count toward this counter (handled by the New-binary auto-rollback rules). Stopping the core uses SIGTERM with a 5 s grace period, then SIGKILL.
