...
- 新增 `tests/webui-e2e/` 独立 pnpm 包：`@playwright/test` + TypeScript，页面对象（登录、workbench、审批卡）与 API helper（seed / 断言 / 会话 key 取用）。
...
- 新增 `invoke webui-e2e` 任务：cargo build（build.rs 自动重建 dist）→ 起沙箱 → 跑 playwright → 清理。
...
- CI 接入（先本地 `invoke webui-e2e`，稳定后另立变更）
