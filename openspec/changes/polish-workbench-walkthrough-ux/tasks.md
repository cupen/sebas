...
- [x] 6.1 排查 Playwright 语义点击全量超时根因（优先验证残留弹窗遮罩假设；其次持续重渲染），结论记入本文件备注；至少让 Add project / History 组头两处语义点击在 e2e 中可用
  ...
  - 遗留（本 change 范围外，需另行立项）：rail 内容宽度失控 flake——`wa-split-panel.frame` 的 `position` 偶发算成 `Infinity`（`position-in-pixels="280"` 已正确绑定，`--rail-width` 未定义），生成非法 `Infinity%` 的 `grid-template-columns`，布局崩坏为单轨道 `1440px`，rail 铺满全宽且刷新不恢复；同一会话内 rail 宽度也观察到在 227↔1390 之间自行变化。现象与本次改动的锚点/文案路径无关，归属（既有 vs 新引入）未定论，需专案定位。
