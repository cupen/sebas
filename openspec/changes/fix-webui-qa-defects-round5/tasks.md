...
- [x] 6.5 GUI 证据补齐（真浏览器，主 agent 整体验收阶段执行）：场景 A（排队重排/移除）、C（API 写 label 行名即时更新）截图入 `.openspec/round5-evidence/`；菜单关闭态 a11y 快照一份。验证标准：证据文件落盘并在本任务行记录
  - 主 agent 负责
  - 状态（2026-09-20 browser-use 验收）：证据已落盘——`scenario-A-queue-stack.png`（排队栈 45 条，含每条上移/下移/移除钮；GUI 点「上移 z201」后 API 核对 z201/z200 换位、点「移除 z202」后 API 核对条目消失——复合后端转发修复后重排/移除真实生效）；`scenario-C-label-live-update.png`（API POST label 后行名 z227 →「验收标签-C」免刷新，帧触发重取链路实证）；`row-menu-open-a11y.txt` + `row-menu-closed-a11y.txt`（开启态含重命名/归档 menuitem、关闭态 a11y 树无菜单项——4.1 对照证据）。附带回归：round3 1.1 首击创建、6.1 聚焦不出徽章/非聚焦出徽章/点击清零/重复点击保持、6.2 模式下拉五选项单行（`round3-6.2-composer-mode-options.png`）、6.3 深链直达标题显示项目名（`round3-6.3-deeplink-title.png`）均通过。
  - 复核（2026-09-21 主 agent 独立二巡，browser-use）：场景 A 复现（「perm」停审批卡冻结队列，8 条栈）——`scenario-a-pending-before/-move/-reorder.png` 三连：GUI 点「上移 hold-b08」后 API 核对 5↔4 换位一致、GUI 点「移除 hold-b03」后 API 核对 7 条一致；场景 C 复现——POST label 后 rail 行名免刷新翻为「live-update-from-qa」（`scenario-c-api-label-live-update.png`）；round3 徽章三态（`badge-focused-arrival-no-badge.png`/`badge-clears-on-refocus.png`）、深链标题（`deeplink-session-title.png`）、模式下拉单行复核一致。观察：hidden-tab 期间到达切回无徽章（聚焦+可见即抑制，与 delta 字面一致；「隐藏期到达零提示」UX 缺口记 bd 跟进）。工具备注：wa-select/pending-stack 的 shadow 组件上 Playwright click actionability 会卡死，CUA 坐标点击正常——测试基建注意项，非产品缺陷。
