...
Lets users open a git repository as a project in the WebUI, spawn a Claude Code
agent in that directory, and interact with it through the browser — turning the
WebUI from a session viewer into a project-oriented agent workspace.
...
The `SessionRow` model SHALL carry `project_dir: Option<String>` and
`prompt_preview: Option<String>` fields. The agent page template SHALL display
