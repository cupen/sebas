...
### D2. 测试包位置：tests/testsuite-webui 独立 pnpm 包
...
- `invoke testsuite-webui`（tasks.py）：`cargo build --bin sebas --bin fake-claude`（build.rs 自动保 dist 新鲜）→ 装 playwright 依赖（缺则 `pnpm install` + `playwright install chromium`，预检并给出清晰报错）→ 跑双 config → 失败保留现场。支持 `--case` 过滤单个旅程（透传 playwright 文件过滤）。
...
- [Windows named pipe 256 字符上限撞长临时路径] → Windows 下沙箱目录名取短（如 `%TEMP%/sbtestsuiteXXXX`）；unix UDS 不受影响。
