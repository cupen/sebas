...
- [ ] 1.1 Create the `sebas-agent` workspace member with modules `llm/` `loop/`
...
- [ ] 1.2 Define the message/tool data model: `Message`, content blocks
...
- [ ] 2.1 Implement the `LlmClient` trait and `FakeLlmClient` in both scripted
...
- [ ] 2.2 Implement `AnthropicMessagesClient`: Anthropic Messages streaming
...
- [ ] 2.3 Map stream events to the crate event type with `TextDelta` /
...
- [ ] 3.1 Implement the `Tool` trait with `ToolCtx` (workdir, cancellation
      ...
      JSON schemas
- [ ] 3.2 Implement bash with process-group spawn, killpg on timeout and
...
- [ ] 3.3 Implement read (line-numbered, offset/limit paging, binary/dir
...
- [ ] 3.4 Implement glob (walkdir + globset, mtime order, 100-cap with
...
- [ ] 4.1 Implement the turn state machine (Idle → AwaitingModel ⇄
...
- [ ] 4.2 Implement the three budgets (model calls, tool calls, wall-clock)
...
- [ ] 4.3 Implement cancellation: token propagation into `ToolCtx`, process
...
- [ ] 5.1 Implement `SessionManager` / `SessionHandle` with per-session task,
...
- [ ] 5.2 Implement system-prompt assembly: sebas-agent identity section +
...
- [ ] 5.3 Map crate-internal events onto the existing `AcpEvent` vocabulary
...
- [ ] 6.1 Add `examples/agent-dev.rs`: headless host reading prompt and
...
- [ ] 6.2 Integration tests in `sebas-agent/tests/`: scripted
...
- [ ] 7.1 Run the full gate: `cargo build` workspace-wide, `cargo test -p
...
- [ ] 7.2 Consistency pass: spec scenarios ↔ test names, design N1–N10 ↔ code
