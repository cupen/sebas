...
- [x] 3.3 服务安装任务：以部署用户 + 绝对 config 路径执行 `sebas service --install`，仅该任务 `become: true`，按 3.2 结论处理幂等；服务未装时才装、装过则收敛。验证：`systemctl is-active sebas` = active，`systemctl show -p User sebas` = 部署用户，ExecStart 为 watchdog 入口；重跑 play 不报错

## 4. 健康检查与幂等收敛

- [x] 4.1 部署后健康检查：`systemctl is-active` + `uri` GET `http://127.0.0.1:{{ sebas_webui_port }}/health`，带启动重试窗（如 10 次 × 3s），rescue 里 assert 报出具体失败断言与观测值。验证：正常部署 playbook 绿；把 `sebas_webui_port` 改成被占用/错误端口重跑，playbook 红且错误信息指明是哪条断言、观测到什么
- [x] 4.2 幂等收敛：输入不变重跑一遍，`changed=0` 且 service 未重启（对比 `ActiveEnterTimestamp`）；改一个 config 变量（如 webui port）重跑，config 更新且恰好一次 restart（handler 通知一次、时间戳变化一次）。验证：两段命令输出分别满足上述断言
...
- [x] 5.2 端到端收尾：在 WSL/Linux 目标机以默认 inventory 完整跑通 release 模式部署（systemd active + `/health` 200 + 幂等重跑），`ansible-playbook site.yml --syntax-check` 与 `ansible-lint`（若可用）通过；conventional commit。验证：完整执行记录（playbook recap 两遍 + 健康检查输出）附在提交说明或 PR 描述中
