...
- [ ] 1.1 `src/cli.rs` 删 `run` 的 `--router`/`--debug` 旗标；`src/run.rs`
...
- [ ] 1.2 全仓清点 `--router` 残留引用（`tests/`、`tasks.py`、脚本），
  逐处迁移或删除；`cargo test --workspace` 相关用例绿

## 2. router 停止保护（watchdog + core）

- [ ] 2.1 core：session channel snapshot 增 `router_activity` domain
...
- [ ] 2.2 control wire：`ServiceSet` 增 `force: bool`（serde 默认 false，
...
- [ ] 2.3 webui 后端 admin adapter 透传 force 与拒绝载荷（HTTP 语义：
  拒绝 → 400 + code + count）；`gateway_bff_test.rs`/api 测试补透传用例

## 3. 前端服务页强制出口

- [ ] 3.1 `settings-modal.ts` services 分区：router 停止被拒（code
  `active_routed_sessions`）→ 二层对话框（计数 + 中断后果文案 +
  「强制停止」/取消），强制走 force 重发；取消不发请求；
  `settings-modal.test.ts` 补三态用例（直接成功/拒绝后强制/拒绝后取消）
- [ ] 3.2 `pnpm --dir sebas-webui/frontend test` 全绿

## 4. 测试设施与文档

- [ ] 4.1 `tasks.py` 浏览器套件装配改两进程（core + `sebas router
  --config <SB>/config.toml --debug`），清理 SIGTERM 两进程；
  `invoke testsuite-webui` 冒烟通过
- [ ] 4.2 e2e `testsuite_e2e_test.rs` 装配同步两进程化；
  `invoke testsuite-e2e` 通过
- [ ] 4.3 AGENTS.md 沙箱菜谱改两进程写法；`config/config.toml.example`
