use sebas_feishu::cards::{derive_topic, render_help_card, render_permission_card, render_root_card};
...
fn card_config_thinking_default_is_show() {
    ...
    assert_eq!(cfg.thinking, sebas_feishu::cards::ThinkingDisplay::Show);
}
...
fn card_config_deserializes_thinking_from_lowercase() {
    ...
    let cfg: sebas_feishu::cards::CardConfig = serde_json::from_value(v).unwrap();
    assert_eq!(cfg.thinking, sebas_feishu::cards::ThinkingDisplay::Hide);
}

#[test]
fn card_config_rejects_unknown_thinking_value() {
    let v = serde_json::json!({ "thinking": "disable" });
    let res: Result<sebas_feishu::cards::CardConfig, _> = serde_json::from_value(v);
    assert!(
        res.is_err(),
...
fn card_config_rejects_unknown_field() {
    ...
    let res: Result<sebas_feishu::cards::CardConfig, _> = serde_json::from_str(json);
    assert!(
        res.is_err(),
...
fn card_config_accepts_all_known_fields() {
    ...
    let c: sebas_feishu::cards::CardConfig = serde_json::from_str(json).unwrap();
    ...
    assert_eq!(c.thinking, sebas_feishu::cards::ThinkingDisplay::Hide);
}
...
fn render_accumulated_card_structure() {
    use sebas_feishu::cards::{CardElement, render_accumulated_card};
    let body = vec![
...
fn render_accumulated_card_with_footer_shows_model_and_tokens() {
    use sebas_feishu::cards::{CardFooter, render_accumulated_card};
