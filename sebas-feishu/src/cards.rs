...
///
/// `deny_unknown_fields`: 手写 `[card]` TOML 段（以及状态库里的 `card_config`
/// 快照）写错字段名（比如 `theme` 而非 `theme_color`）必须立刻报错，而不是被
/// serde 静默忽略后被下一次 save_settings 抹平。
    ...
    fn serialize<S: serde::Serializer>(&self, ser: S) -> Result<S::Ok, S::Error> {
        ...
        match self {
            ...
            CardElement::SelectStatic {
                ...
                // from `sebas_feishu::forms` (tag, placeholder, options, behaviors,
