//! 统一的「启动失败」退出路径（fail-fast-on-startup-errors），**两个二进制共用**：
//! 主控 `sebas` 与执行节点 `sebas-node`（add-remote-execution-node D0）。
//!
//! 规约（openspec/changes/archive/2026-09-08-fail-fast-on-startup-errors）：任何
//! sebas 进程在达到 ready 之前发生 fatal 时 SHALL：
//!
...
//!
//! 该文件同时是「最近一次启动尝试失败」的闩锁：进程成功达到 ready 后调用
//! [`clear_env_summary_file`] 删除它，这样下游客户端在服务不可达时读到的
//! 摘要是本次启动尝试的真实结果，不会把陈旧失败报给已恢复的部署。
//!
//! 本 crate 是叶子依赖：不含任何主控角色实现（core / webui / router / im），
//! 因此节点二进制可以安全依赖它——这正是它从主控 crate 里抽出来的原因。

/// 启动失败的保留退出码（EX_TEMPFAIL）。与 watchdog 的 `EXIT_BIND_FAILED`
...
pub fn write_env_summary_file(cause: &str) {
    ...
    if let Err(e) = std::fs::write(std::path::PathBuf::from(&path), summary_line(cause)) {
        eprintln!(
            "warning: cannot write {SUMMARY_FILE_ENV}={}: {e}",
...
mod tests {
    ...
    fn read_env_summary_returns_cause_after_prefix() {
        ...
        .unwrap();
        let _guard = set_env_file(&path);
        assert_eq!(read_env_summary(), Some("bind failed: addr in use".into()));
        drop(_guard);
    }
