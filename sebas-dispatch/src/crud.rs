...
impl ProviderForms {
    ...
    /// 两个按钮以及双 form 调度。
    pub async fn cancel(&self, key: ChannelKey, message_id: Option<String>) -> crate::engine::Out {
    ...
    /// 渲染一张带两个「＋ 新增」按钮的列表卡。
    pub async fn open(&self, key: ChannelKey) -> crate::engine::Out {
        self.render_list_card(key).await
    }

    async fn render_list_card(&self, key: ChannelKey) -> crate::engine::Out {
        let card = self.build_list_card().await;
        // 列表卡没有 context open_message_id：新发一张。
        crate::engine::Out::SendCard {
            key,
            card,
            msg_id: None,
            perm_request_id: None,
