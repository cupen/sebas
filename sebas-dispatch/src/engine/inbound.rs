...
/// via `DispatchHandle::record_help_card_msgid` for later in-place updates.
...
impl DispatchHandle {
    ...
    async fn on_text(&self, key: ChannelKey, text: String, reply_to: Option<String>) {
        ...
        match parse_command(&text) {
            ...
            Command::Router(action) => {
                self.request_watchdog_router(key, action).await;
            }
            Command::Webui => {
    ...
    async fn request_watchdog_router(&self, key: ChannelKey, action: RouterAction) {
        self.emit(Out::WatchdogRouter { key, action }).await;
    }
