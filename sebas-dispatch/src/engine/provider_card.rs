...
//!
//! 1. **Mode segmented control** — Off / Direct / Router 三按钮。
...
pub async fn render_main_card(handle: &DispatchHandle, key: &ChannelKey) -> Out {
...
/// 是 spawn 翻译的权威值。
async fn build_default_selection(handle: &DispatchHandle, name: &str) -> DefaultSelection {
...
async fn handle_back(handle: &DispatchHandle, key: &ChannelKey, message_id: Option<String>) -> Out {
    refresh_card(handle, key, message_id).await
}
...
fn mode_display_label(mode: &ProviderMode) -> String {
    match mode {
        ...
        ProviderMode::Router => "Router".into(),
    }
}

/// 三个模式按钮。当前模式高亮（primary），其余 default。
/// 按顺序：Off / Direct / Router。`behaviors[].value` 携带
...
mod tests {
    ...
    fn handle_with(
        ...
        let (h, _rx) = DispatchHandle::new_with_provider_form(
            SessionMap::new(),
            Default::default(),
            16,
            Some(Arc::new(forms)),
    ...
    async fn mode_buttons_highlight_current_mode() {
        ...
        assert_eq!(types.get("router").map(String::as_str), Some("default"));

        // 2) 切到 Router → Router primary。
        provider_state::update(|s| s.mode = ProviderMode::Router).unwrap();
        let out = render_main_card(&handle, &test_key()).await;
        let card = match out {
            Out::SendCard { card, .. } => card,
            _ => panic!(),
        ...
        let types = button_mode_types(&card);
        assert_eq!(types.get("router").map(String::as_str), Some("primary"));
    ...
    async fn dispatch_route_for_mode_updates_state_and_refreshes() {
        ...
        let payload = json!({ "form": FORM_MODE, "mode": "router" });
