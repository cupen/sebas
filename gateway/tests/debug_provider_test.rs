...
//!
//! `[gateway] debug = true` 时，model=test 的请求由 gateway 自身应答：
...
use support::{start_gateway, start_gateway_debug};

/// debug 配置：parse 后由 `start_gateway_debug` 注入内置 test provider；
...
#[tokio::test]
async fn non_debug_gateway_does_not_intercept_test_model() {
    let gw = start_gateway(NON_DEBUG_CFG).await;
    let resp = client()
        .post(format!("http://{}/v1/messages", gw.addr))
        .header("authorization", "Bearer sk-gw-debug")
        .header("anthropic-version", "2023-06-01")
