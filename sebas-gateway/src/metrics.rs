...
/// 与内核热替换无关；多个 gateway 实例（测试）共享计数在测试里做相对
/// 断言（前后差值）即可，生产恒单实例。
pub struct Metrics {
...
impl Metrics {
    ...
    pub fn observe_request(
        ...
        self.inc(&format!(
            "sebas_gateway_requests_total{{provider=\"{provider}\",model=\"{}\"}}",
        ...
        for b in LATENCY_BUCKETS_MS {
            if ms <= b {
                self.inc(&format!(
                    "sebas_gateway_request_duration_ms_bucket{{provider=\"{provider}\",le=\"{b}\"}}"
                ));
            }
        }
        self.inc(&format!(
            "sebas_gateway_request_duration_ms_count{{provider=\"{provider}\"}}"
        ));
        self.add(
            &format!("sebas_gateway_tokens_total{{provider=\"{provider}\",kind=\"input\"}}"),
            input_tokens,
        );
        self.add(
            &format!("sebas_gateway_tokens_total{{provider=\"{provider}\",kind=\"output\"}}"),
            output_tokens,
        );
        if status >= 500 {
            self.inc(&format!(
                "sebas_gateway_upstream_errors_total{{provider=\"{provider}\"}}"
            ));
        }
    }
    ...
    pub fn observe_auth_rejected(&self) {
        self.inc("sebas_gateway_auth_rejected_total");
    }

    /// rate-limit 拒绝（429）。
    pub fn observe_rate_limited(&self) {
        self.inc("sebas_gateway_rate_limited_total");
    }
...
const ACTIVE_SERIES: &str = "sebas_gateway_active_requests";
...
mod tests {
    ...
    fn three_requests_count_three() {
        ...
        assert_eq!(
            m.get("sebas_gateway_requests_total{provider=\"alpha\",model=\"m1\"}"),
            3
        );
        assert_eq!(
            m.get("sebas_gateway_tokens_total{provider=\"alpha\",kind=\"input\"}"),
            30
        );
        // 直方图桶：50ms ≤ 50/100/... 都累计 3。
        assert_eq!(
            m.get("sebas_gateway_request_duration_ms_bucket{provider=\"alpha\",le=\"50\"}"),
            3
        );
        assert_eq!(
            m.get("sebas_gateway_request_duration_ms_bucket{provider=\"alpha\",le=\"10\"}"),
            0
        );
    }
    ...
    fn rate_limited_counts() {
        ...
        m.observe_rate_limited();
        assert_eq!(m.get("sebas_gateway_rate_limited_total"), 2);
    }
    ...
    fn series_cap_merges_to_other() {
        ...
        assert_eq!(
            m.get("sebas_gateway_requests_total{provider=\"alpha\",model=\"other\"}"),
            1
        );
    }
